...
//def call(String buildStatus = 'STARTED') {
  // build status of null means successful
    environment {
          id='768a8310-7348-11ea-8dca-b5cc1c3a792c'
    }
...
if ( buildStatus == "SUCCESS" ) {
 ...
 sparkSend ( credentialsId: 'sparkbot',  message: ' success - ${JOB_NAME}-${BUILD_NUMBER} - ${BUILD_RESULT} - ${JOB_URL}', messageType: 'markdown', spaceList: [[spaceId: '${env.id}', spaceName: 'common']] ) 
}
  else if( buildStatus == "FAILURE" ) { 
sparkSend ( credentialsId: 'sparkbot',  message: 'failure - ${JOB_NAME}-${BUILD_NUMBER} - ${BUILD_RESULT} - ${JOB_URL}', messageType: 'markdown', spaceList: [[spaceId: '${env.id}', spaceName: 'common']] )
  }
